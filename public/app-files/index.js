...
(function() {
  ...
  var scenes = data.scenes.map(function(data) {
    ...
    data.infoHotspots.forEach(function(hotspot) {
      ...
      scene.hotspotContainer().createHotspot(element, { yaw: hotspot.yaw, pitch: hotspot.pitch });
      infoHotspotElements.push(element);
    });
  ...
  function closeAllInfoHotspots() {
    infoHotspotElements.forEach(function(element) {
      if (!element) {
        return;
      }
      element.classList.remove('visible');
      if (element._hotspotModal) {
  ...
  function openInfoHotspotByTitle(title) {
    ...
    var target = infoHotspotElements.find(function(element) {
      return element && element.dataset && element.dataset.hotspotTitle === title;
    });
    if (!target) {
      return;
    }
  ...
  function lookAt(params) {
    if (!currentScene) {
      return;
    }
    var viewParams = currentScene.view.parameters();
    currentScene.view.setParameters({
      yaw: params.yaw !== undefined ? params.yaw : viewParams.yaw,
      pitch: params.pitch !== undefined ? params.pitch : viewParams.pitch,
      fov: params.fov !== undefined ? params.fov : viewParams.fov
    });
  }

  window.addEventListener('message', function(event) {
    var payload = event.data;
    if (!payload || payload.type !== 'panorama:lookAt') {
      return;
    }
    lookAt(payload);
    // Always close all info modals before opening the requested one
    if (payload.openHotspotTitle) {
      closeAllInfoHotspots();
      setTimeout(function() {
        openInfoHotspotByTitle(payload.openHotspotTitle);
      }, 50); // Small delay to ensure close then open
    }
  });
  ...
  function createInfoHotspotElement(hotspot) {
    ...
    wrapper.dataset.hotspotTitle = hotspot.title;
